...
void EOS_CALL EOSLog(const EOS_LogMessage* InMsg)
{
	...
	case EOS_ELogLevel::EOS_LOG_Fatal:
		{
			LogEOSError("%s: %s", ANSI_TO_TCHAR(InMsg->Category), ANSI_TO_TCHAR(InMsg->Message));
	...
	case EOS_ELogLevel::EOS_LOG_Warning:
		{
			LogEOSWarning("%s: %s", ANSI_TO_TCHAR(InMsg->Category), ANSI_TO_TCHAR(InMsg->Message));
	...
	case EOS_ELogLevel::EOS_LOG_Verbose:
		{
			LogEOSVerbose("%s: %s", ANSI_TO_TCHAR(InMsg->Category), ANSI_TO_TCHAR(InMsg->Message));
			UE_LOG_ONLINE(Verbose, TEXT("EOSSDK-%s: %s"), ANSI_TO_TCHAR(InMsg->Category), ANSI_TO_TCHAR(InMsg->Message));
			break;
		}
	case EOS_ELogLevel::EOS_LOG_VeryVerbose:
		{
			LogEOSVeryVerbose("%s: %s", ANSI_TO_TCHAR(InMsg->Category), ANSI_TO_TCHAR(InMsg->Message));
...
{
	UE_LOG_ONLINE(Log, TEXT("FOnlineSubsystemEOSCore::PlatformCreate"));
	...
	{
		LogEOSWarning("CacheDir: %s invalid, attempting to use Project Saved Dir..", *CacheDir);
	...
	{
		LogEOSVerbose("Overriding CacheDir: %s", *OverrideCacheDir);
	...
	LogEOSVerbose("Using CacheDir: %s", *CacheDir);
	...
	bool bDisableEOSOverlayViaCommandline = false;
	if (FParse::Bool(FCommandLine::Get(), TEXT("DisableEOSOverlay"), bDisableEOSOverlayViaCommandline))
	{
		bDisableEOSOverlay = bDisableEOSOverlayViaCommandline;
	}
	...
	if (EOSConfig.bEnableRTC == false)
	{
		...
		LogEOSVerbose("RTC disabled");
	}
	else
	{
		LogEOSVerbose("RTC enabled (%s)", *Path);
	}

	if (EOSConfig.ProductId.Len() == 0)
	{
		LogEOSError("EOSCore missing ProductId in DefaultEngine.ini");
	}

	if (EOSConfig.SandboxId.Len() == 0)
	{
		LogEOSError("EOSCore missing SandboxId in DefaultEngine.ini");
	}

	if (EOSConfig.DeploymentId.Len() == 0)
	{
		LogEOSError("EOSCore missing DeploymentId in DefaultEngine.ini");
	}

	if (EOSConfig.ClientId.Len() == 0)
	{
		LogEOSError("EOSCore missing ClientId in DefaultEngine.ini");
	}

	if (EOSConfig.ClientSecret.Len() == 0)
	{
		LogEOSError("EOSCore missing ClientSecret in DefaultEngine.ini");
	}

	if (EOSConfig.EncryptionKey.Len() == 0)
	{
		LogEOSError("EOSCore missing EncryptKey in DefaultEngine.ini");
	}
	...
	if (!PlatformOptions.IntegratedPlatformOptionsContainerHandle)
	{
		...
		{
			LogEOSError("Failed to create integrated platform options container", ANSI_TO_TCHAR(EOS_EResult_ToString(Result)));
		}
	}
	
	static double TaskNetworkTimeoutSeconds = 10.f;
	PlatformOptions.TaskNetworkTimeoutSeconds = &TaskNetworkTimeoutSeconds;
...
bool FOnlineSubsystemEOSCore::Init()
{
	...
	if ((IsRunningGame() || IsRunningDedicatedServer()) && GEOSPlatformHandle != nullptr)
	{
		m_EOSPlatformHandle = GEOSPlatformHandle;
	}
	else
	{
		UE_LOG_ONLINE(Log, TEXT("Creating PlatformHandle"));
		m_EOSPlatformHandle = FOnlineSubsystemEOSCore::PlatformCreate();
	}

	if (m_EOSPlatformHandle == nullptr)
	{
		UE_LOG_ONLINE(Warning, TEXT("m_EOSPlatformHandle was nullptr"));
		return false;
	}
	...
	{
		UE_LOG_ONLINE(Warning, TEXT("FOnlineSubsystemEOSCore: couldn't get rtc handle"));
	}

	m_HRTCAdminHandle = EOS_Platform_GetRTCAdminInterface(m_EOSPlatformHandle);
	if (m_HRTCAdminHandle == nullptr)
	{
		UE_LOG_ONLINE(Warning, TEXT("FOnlineSubsystemEOSCore: couldn't get rtc admin handle"));
	}
