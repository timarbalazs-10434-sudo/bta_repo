...
class FSocketEOSCore : public FSocket
{
public:
	FSocketEOSCore(FSocketSubsystemEOSCore& SocketSubsystem, const FString& InSocketDescription);
	virtual ~FSocketEOSCore();
public:
...
public:
	void SetLocalAddress(const FInternetAddrEOSCore& InLocalAddress);
	bool Close(const FInternetAddrEOSCore& RemoteAddress);
	void RegisterClosedNotification();

	bool WasClosed(const FInternetAddrEOSCore& RemoteAddress)
	{
		int32 Index = -1;
...
private:
	bool bIsListening;
	FSocketSubsystemEOSCore& m_SocketSubsystem;
	FInternetAddrEOSCore m_LocalAddress;
	TArray<FInternetAddrEOSCore> m_ClosedRemotes;
