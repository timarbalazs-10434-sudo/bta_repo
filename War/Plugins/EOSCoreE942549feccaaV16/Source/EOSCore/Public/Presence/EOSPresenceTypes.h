...
public:
		...
		, RecordsCount(data.RecordsCount)
	{
		...
		const FUTF8ToTCHAR ProductVersionChar(data.ProductVersion);
		ProductVersion = ProductVersionChar.Get();

		const FUTF8ToTCHAR PlatformChar(data.Platform);
		Platform = PlatformChar.Get();

		const FUTF8ToTCHAR RichTextChar(data.RichText);
		RichText = RichTextChar.Get();

		const FUTF8ToTCHAR ProductNameChar(data.ProductName);
		ProductName = ProductNameChar.Get();
...
public:
	...
	{
		const FUTF8ToTCHAR JoinInfoChar(data.JoinInfo);
		JoinInfo = JoinInfoChar.Get();
	}
};
...
public:
	FQueryPresenceCompleteCallback(const FOnPresenceQueryPresenceCompleteCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FQueryPresenceCompleteCallback()
	{
		m_Callback.Unbind();
	}
};
...
public:
	FSetPresenceCompleteCallback(const FOnPresenceSetPresenceCompleteCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FSetPresenceCompleteCallback()
	{
		m_Callback.Unbind();
	}
};
...
{
public:
	FPresenceOnJoinGameAcceptedCallback(const FOnPresenceOnJoinGameAcceptedCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FPresenceOnJoinGameAcceptedCallback()
	{
		m_Callback.Unbind();
	}
public:
	FOnPresenceOnJoinGameAcceptedCallback m_Callback;
};

struct FPresenceChangedCallback
{
public:
	FPresenceChangedCallback(const FOnPresenceChangedCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FPresenceChangedCallback()
	{
		m_Callback.Unbind();
	}
public:
	FOnPresenceChangedCallback m_Callback;
};
