...
FOnlineStoreEOSCore::FOnlineStoreEOSCore(FOnlineSubsystemEOSCore* InSubsystem)
	: m_Subsystem(InSubsystem)
{
	check(m_Subsystem != nullptr);
}

void FOnlineStoreEOSCore::QueryCategories(const FUniqueNetId& UserId, const FOnQueryOnlineStoreCategoriesComplete& Delegate)
{
	Delegate.ExecuteIfBound(false, TEXT("QueryCategories Not Implemented"));
}

void FOnlineStoreEOSCore::GetCategories(TArray<FOnlineStoreCategory>& OutCategories) const
{
	OutCategories.Reset();
}

void FOnlineStoreEOSCore::QueryOffersByFilter(const FUniqueNetId& UserId, const FOnlineStoreFilter& Filter, const FOnQueryOnlineStoreOffersComplete& Delegate)
{
	QueryOffers(UserId, Delegate);
}

void FOnlineStoreEOSCore::QueryOffersById(const FUniqueNetId& UserId, const TArray<FUniqueOfferId>& OfferIds, const FOnQueryOnlineStoreOffersComplete& Delegate)
{
	QueryOffers(UserId, Delegate);
}
...
void FOnlineStoreEOSCore::QueryOffers(const FUniqueNetId& UserId, const FOnQueryOnlineStoreOffersComplete& Delegate)
{
...
void FOnlineStoreEOSCore::GetOffers(TArray<FOnlineStoreOfferRef>& OutOffers) const
{
	OutOffers = m_CachedOffers;
}

TSharedPtr<FOnlineStoreOffer> FOnlineStoreEOSCore::GetOffer(const FUniqueOfferId& OfferId) const
{
...
void FOnlineStoreEOSCore::Checkout(const FUniqueNetId& UserId, const FPurchaseCheckoutRequest& CheckoutRequest, const FOnPurchaseCheckoutComplete& Delegate)
{
	EOS_EpicAccountId AccountId = m_Subsystem->m_IdentityInterfacePtr->GetEpicAccountId(UserId);
	if (AccountId == nullptr)
	{
...
void FOnlineStoreEOSCore::Checkout(const FUniqueNetId& UserId, const FPurchaseCheckoutRequest& CheckoutRequest, const FOnPurchaseReceiptlessCheckoutComplete& Delegate)
{
	UE_LOG_ONLINE(Error, TEXT("FOnlineStoreEOSCore::Checkout: Not supported."));
	Delegate.ExecuteIfBound(ONLINE_ERROR(EOnlineErrorResult::NotImplemented));
}
#endif

void FOnlineStoreEOSCore::FinalizePurchase(const FUniqueNetId& UserId, const FString& ReceiptId)
{
	UE_LOG_ONLINE(Error, TEXT("FinalizePurchase: Not supported. Did you mean FinalizeReceiptValidationInfo?"));
}

void FOnlineStoreEOSCore::RedeemCode(const FUniqueNetId& UserId, const FRedeemCodeRequest& RedeemCodeRequest, const FOnPurchaseRedeemCodeComplete& Delegate)
{
...
void FOnlineStoreEOSCore::QueryReceipts(const FUniqueNetId& UserId, bool bRestoreReceipts, const FOnQueryReceiptsComplete& Delegate)
{
	EOS_EpicAccountId AccountId = m_Subsystem->m_IdentityInterfacePtr->GetEpicAccountId(UserId);
	if (AccountId == nullptr)
	{
...
void FOnlineStoreEOSCore::GetReceipts(const FUniqueNetId& UserId, TArray<FPurchaseReceipt>& OutReceipts) const
{
...
void FOnlineStoreEOSCore::FinalizeReceiptValidationInfo(const FUniqueNetId& UserId, FString& InReceiptValidationInfo, const FOnFinalizeReceiptValidationInfoComplete& Delegate)
{
	EOS_EpicAccountId AccountId = m_Subsystem->m_IdentityInterfacePtr->GetEpicAccountId(UserId);
	if (AccountId == nullptr)
	{
	...
	CallbackObj->m_CallbackLambda = [this, Info = FString(InReceiptValidationInfo), OnComplete = FOnFinalizeReceiptValidationInfoComplete(Delegate)
...
bool FOnlineStoreEOSCore::HandleEcomExec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
		...
		QueryReceipts(*m_Subsystem->m_IdentityInterfacePtr->GetLocalUniqueNetIdEOS(), false,
		              FOnQueryReceiptsComplete::CreateLambda([this](const FOnlineError& Result)
		              {
			              ...
			              for (const FPurchaseReceipt& Receipt : m_CachedReceipts)
			              {
				              ...
				              UE_LOG_ONLINE(Log, TEXT("\tLine item (%s) is %s"), *Receipt.ReceiptOffers[0].LineItems[0].ItemName,
				                            Receipt.ReceiptOffers[0].LineItems[0].IsRedeemable() ? TEXT("redeemable") : TEXT("not redeemable"));
			              }
		              }));
		return true;
	}
