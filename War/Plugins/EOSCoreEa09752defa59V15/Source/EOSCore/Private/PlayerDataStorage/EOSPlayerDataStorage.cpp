...
static size_t MaxChunkSize = 4 * 4 * 4096;
...
void UCorePlayerDataStorage::EOSPlayerDataStorageReadFile(UObject* WorldContextObject, FEOSPlayerDataStorageReadFileOptions ReadOptions, const FOnReadFileCompleteCallback& Callback, const FOnFileTransferProgressUpdated& ProgressCallback)
{
	LogEOSVerbose("");
	...
	if (GetPlayerDataStorageHandle(WorldContextObject))
	{
		...
		PlayerDataStorageOptions.ReadFileDataCallback = &ThisClass::OnFileDataReceived;
		PlayerDataStorageOptions.FileTransferProgressCallback = &ThisClass::OnFileTransferProgressUpdated;

		s_CurrentRead.LocalFile = ReadOptions.LocalFile;
		FReadFileCompleteCallback* CallbackObj = new FReadFileCompleteCallback(this, Callback, ProgressCallback);
...
void UCorePlayerDataStorage::EOSPlayerDataStorageWriteFile(UObject* WorldContextObject, FEOSPlayerDataStorageWriteFileOptions WriteOptions, const FOnWriteFileCompleteCallback& Callback, const FOnFileTransferProgressUpdated& ProgressCallback)
{
	LogEOSVerbose("");
	...
	if (GetPlayerDataStorageHandle(WorldContextObject))
	{
		...
		FWriteFileCompleteCallback* Data = new FWriteFileCompleteCallback(this, Callback, ProgressCallback);
...
{
	if (FReadFileCompleteCallback* CallbackObj = static_cast<FReadFileCompleteCallback*>(Data->ClientData))
	{
		CallbackObj->m_OnFileTransferProgressUpdated.ExecuteIfBound(*Data);
	}

	if (FWriteFileCompleteCallback* CallbackObj = static_cast<FWriteFileCompleteCallback*>(Data->ClientData))
	{
		CallbackObj->m_OnFileTransferProgressUpdated.ExecuteIfBound(*Data);
	}
