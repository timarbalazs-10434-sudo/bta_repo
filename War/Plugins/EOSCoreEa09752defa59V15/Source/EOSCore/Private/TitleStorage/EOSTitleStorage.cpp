...
FEOSTitleStorageFileTransferRequestHandle UCoreTitleStorage::EOSTitleStorageReadFile(UObject* WorldContextObject, FEOSTitleStorageReadFileOptions Options, const FOnTitleStorageReadFileCompleteCallback& Callback, const FOnTitleStorageFileTransferProgressCallback& OnProgressCallback)
{
	LogEOSVerbose("");
	...
	if (GetTitleStorageHandle(WorldContextObject))
	{
		...
		TitleStorageOptions.ReadFileDataCallback = &ThisClass::OnReadFileDataCallback;
		TitleStorageOptions.FileTransferProgressCallback = &ThisClass::OnFileTransferProgressCallback;

		FReadFileCallback* CallbackObj = new FReadFileCallback(this, Callback, OnProgressCallback);
...
void UCoreTitleStorage::OnFileTransferProgressCallback(const EOS_TitleStorage_FileTransferProgressCallbackInfo* Data)
{
	...
	if (CallbackObj)
	{
		...
		CallbackObj->m_ProgressCallback.ExecuteIfBound(*Data);
	}
}
