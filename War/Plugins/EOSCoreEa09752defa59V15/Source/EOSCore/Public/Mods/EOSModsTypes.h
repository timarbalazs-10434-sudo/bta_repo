...
public:
	...
	{
		const FUTF8ToTCHAR NamespaceIdChar(data.NamespaceId);
		NamespaceId = NamespaceIdChar.Get();

		const FUTF8ToTCHAR ItemIdChar(data.ItemId);
		ItemId = ItemIdChar.Get();

		const FUTF8ToTCHAR ArtifactIdChar(data.ArtifactId);
		ArtifactId = ArtifactIdChar.Get();

		const FUTF8ToTCHAR TitleChar(data.Title);
		Title = TitleChar.Get();

		const FUTF8ToTCHAR VersionChar(data.Version);
		Version = VersionChar.Get();
	}
};
...
{
public:
	FOnUpdateModCallback m_Callback;
public:
	FUpdateModCallback(const FOnUpdateModCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FUpdateModCallback()
	{
		m_Callback.Unbind();
	}
};
...
{
public:
	FOnEnumerateModsCallback m_Callback;
public:
	FEnumerateModsCallback(const FOnEnumerateModsCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FEnumerateModsCallback()
	{
		m_Callback.Unbind();
	}
};
...
{
public:
	FOnUninstallModCallback m_Callback;
public:
	FUninstallModCallback(const FOnUninstallModCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FUninstallModCallback()
	{
		m_Callback.Unbind();
	}
};
...
{
public:
	FOnInstallModCallback m_Callback;
public:
	FInstallModCallback(const FOnInstallModCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FInstallModCallback()
	{
		m_Callback.Unbind();
	}
};
