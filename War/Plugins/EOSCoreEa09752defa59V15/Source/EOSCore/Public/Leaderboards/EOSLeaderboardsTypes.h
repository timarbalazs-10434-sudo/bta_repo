...
public:
	...
	FEOSLeaderboardsDefinition(const EOS_Leaderboards_Definition& data)
		...
		, EndTime(LexToString(data.EndTime))
	{
		const FUTF8ToTCHAR LeaderboardIdChar(data.LeaderboardId);
		LeaderboardId = LeaderboardIdChar.Get();
		
		const FUTF8ToTCHAR StatNameChar(data.StatName);
		StatName = StatNameChar.Get();
	}
};
...
public:
	...
	FEOSLeaderboardsLeaderboardRecord(const EOS_Leaderboards_LeaderboardRecord& data)
		...
		, Score(data.Score)
	{
		const FUTF8ToTCHAR UserDisplayNameChar(data.UserDisplayName);
		UserDisplayName = UserDisplayNameChar.Get();
	}
};
...
public:
	FLeaderboardDefinitionsCompleteCallback(const FOnQueryLeaderboardDefinitionsCompleteCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FLeaderboardDefinitionsCompleteCallback()
	{
		m_Callback.Unbind();
	}
};
...
public:
	FLeaderboardRanksCompleteCallback(const FOnQueryLeaderboardRanksCompleteCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FLeaderboardRanksCompleteCallback()
	{
		m_Callback.Unbind();
	}
};
...
public:
	FLeaderboardUserScoresCompleteCallback(const FOnQueryLeaderboardUserScoresCompleteCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FLeaderboardUserScoresCompleteCallback()
	{
		m_Callback.Unbind();
	}
};
