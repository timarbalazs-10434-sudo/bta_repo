...
public:
	...
	{
		const FUTF8ToTCHAR StatNameIdChar(data.StatName);
		StatName = StatNameIdChar.Get();
	}
};
...
public:
	FIngestStatCompleteCallback(const FOnStatsIngestStatCompleteCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FIngestStatCompleteCallback()
	{
		m_Callback.Unbind();
	}
};
...
public:
	FQueryStatsCompleteCallback(const FOnStatsQueryStatsCompleteCallback& callback)
		: m_Callback(callback)
	{
	}
	virtual ~FQueryStatsCompleteCallback()
	{
		m_Callback.Unbind();
	}
};
...
public:
		...
		: EOS_Stats_QueryStatsOptions()
	{
		...
		StartTime = startTime;
		EndTime = endTime;
		StatNames = const_cast<const char**>(PointerArray.GetData());
